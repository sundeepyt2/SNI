...
def config(
    ...
    if cookie_info:
        ...
        console.print(Panel(
            ...
            "SNI's XAN-port endpoint pattern (POST /api/graphql + GET /api persisted\n"
            "query) avoids Cloudflare's captcha heuristic on its own. Most users\n"
            "never need cookies OR a CF Worker. Just run `sni play \"one piece\"`\n"
            "directly — if it works, you're done.\n\n"
            "[bold]If you DO hit a NEED_CAPTCHA error,[/bold] try these in order:\n\n"
            "[bold cyan]Option 1 - Switch providers (zero setup, fastest fix):[/bold cyan]\n"
            "  sni play 'one piece' -p hianime\n"
            "  HiAnime uses different infrastructure and is rarely captcha-walled.\n\n"
            "[bold yellow]Option 2 - Browser cookies (works if your IP isn't[/bold yellow]\n"
            "[bold yellow]permanently flagged):[/bold yellow]\n"
            "  Get cookies from a working allanime mirror (NOT allanime.day which is\n"
            "  currently broken with a redirect loop). Working mirrors include:\n"
            "    - https://allmanga.to\n"
            ...
            "[bold]Option 3 - Cloudflare Worker (only if you're on a VPN/shared IP[/bold]\n"
            "[bold]that's permanently captcha-walled AND cookies don't work):[/bold]\n"
            "  Deploy the XAN CF Worker (free, but requires a Cloudflare account):\n"
            ...
            "  Adapt the worker.js export style to the platform's expected handler.\n\n"
            "[dim]Note: allanime.day itself is currently broken with a server-side[/dim]\n"
            "[dim]redirect loop ('too many redirects'). This affects the website only[/dim]\n"
            "[dim]- SNI's API endpoint (api.allanime.day) is independent and working.[/dim]\n"
            "[dim]You do NOT need to visit allanime.day to use SNI.[/dim]",
